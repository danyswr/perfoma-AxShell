...
class Bar(Window):
    def __init__(self, monitor_id: int = 0, **kwargs):
        ...
        self.button_ai = Button(
            name="button-bar",
            tooltip_markup=tooltip_ai,
            on_clicked=lambda *_: self.ai_dashboard(),
            child=Label(name="button-bar-label", markup=icons.ai_brain),
        )
        self.button_ai.connect("enter_notify_event", self.on_button_enter)
        self.button_ai.connect("leave_notify_event", self.on_button_leave)
